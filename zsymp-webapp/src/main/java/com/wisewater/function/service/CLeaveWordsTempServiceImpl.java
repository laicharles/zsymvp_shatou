...
@Service
public class CLeaveWordsTempServiceImpl extends BaseService implements CLeaveWordsTempService{
	...
	@Override
	public boolean sendToLeaveWordsPerson(String token, String openId, String nickName,String content) {
...
//		String templateId = findTemplateId(accessTokenForm.getAccessToken(),token);
		//TODO 温馨提示模本ID
		String templateId = loadConstant.getLeavewordsTempCode();
		if("".equals(templateId)){
			return false;
		}
		...
		Client client;
		try{
			String url =loadConstant.getWebServicePath();
			System.out.println(url);
			client = new Client(new URL(url));
			Object[] results = client.invoke("sendLeaveWordsForLeaveWordsPerson", new Object[] {accessTokenForm.getAccessToken(), templateId, token, openId,nickName,content });
