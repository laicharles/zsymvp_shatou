...
<div class="wraper sidebar-full">
    ...
    <div class="container-fluid auto-fluid">
        <h2 class="title_class">增加${dicName}</h2>
		<form:form action="${pageContext.request.contextPath}/cus/ccms_add.do?_csrf=${_csrf.token}" modelAttribute="CmsAddForm" method="POST"  enctype="multipart/form-data">
			<input type="hidden" name="id" id="id" >
			
			<input type="hidden" name="cmsType.logicID" value="${cmsType }">
			<div>
				<label for="titleId"><font color="red">*</font>标题：</label>
				<input type="text" name="title" id="titleId" class="general_input_class">
			</div>
			<div>
				<label for="authorId">作者：</label>
				<input type="text" name="author" id="authorId" class="general_input_class">
			</div>
			...
			<div>
				...
				<input type="text" id="sortNumberId"  class="pageUrl_class" name="sortNumber">
			</div>
			<div>
				<label for="origUrlId">原文链接：</label>
				<input type="text" id="origUrlId"  class="pageUrl_class" name="origUrl">
			</div>
			<div>
				<label for="titleId">正文外链：</label>
				<input type="text" id="pageUrlId"  class="pageUrl_class" name="pageUrl">
...
<script type="text/javascript"  src="<c:url value="/resources/js/uploadPreview.min.js"/>"></script>
...
//     	$("#titleId").val(obj.title);
    ...
    $(function(){
    	...
    	$("#returnBtn").click(function(){
    		window.location.href="${pageContext.request.contextPath}/cus/${cmsType}/ccms_list.do?pageNo=1";
    	});
    	//保存
    		...
    		if(checkSubmit()){
    			bootbox.confirm("确定保存吗？",function(result){
    				if(result==true){
    					...
    					$("#CmsAddForm").ajaxSubmit(function(msg){
               				if(msg.result==false){
               					bootbox.alert(msg.warnMsg,function(){
               						window.location.href="${pageContext.request.contextPath}/cus/"+msg.tips+"/ccms_list.do?pageNo=1";
               					});
            				}else{
            					bootbox.alert(msg.warnMsg,function(){
            						window.location.href="${pageContext.request.contextPath}/cus/"+msg.tips+"/ccms_list.do?pageNo=1";
