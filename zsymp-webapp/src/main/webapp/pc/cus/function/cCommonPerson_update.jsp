...
<div class="wraper sidebar-full">
    ...
    <div class="container-fluid">
        <h2 class="title_class">更新人员</h2>
		<form:form action="${pageContext.request.contextPath}/cus/cCommonPerson_update.do" modelAttribute="cCommonPersonForm" method="POST">
			<input type="hidden" name="id" id="id" value="${cCommonPersonForm.id }">
			<input type="hidden" name="token" value="${cCommonPersonForm.token }">
			<div>
				<label for="openIDId"><font color="red">*</font>openID：</label>
				<input type="text" name="openID" id="openIDId" value="${cCommonPersonForm.openID }" class="general_input_class">
			</div>
			<div>
				<label for="nameId"><font color="red">*</font>备注：</label>
				<input type="text" name="name" id="nameId" value="${cCommonPersonForm.name }"  class="general_input_class">
			</div>
...
<script>
    ...
    $(function(){
    	...
    	$("#returnBtn").click(function(){
    		window.location.href="${pageContext.request.contextPath}/cus/cCommonPerson_list.do?pageNo=1";
    	});
    	//保存
    	$("#saveBtn").click(function(){
    		if(checkSubmit()){
    			bootbox.confirm("确定保存吗？",function(result){
    				if(result==true){
    					$("#cCommonPersonForm").ajaxSubmit(function(msg){
               				if(msg.result==false){
