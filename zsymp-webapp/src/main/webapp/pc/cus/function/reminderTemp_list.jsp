...
<div class="wraper sidebar-full">
    ...
    <div class="container-fluid">
        <!--主体内容-->
        <h2 class="title_class">温馨提示</h2>
		<div class="list_btn_div_class">
        <input type="button" id="addRecord" class="btn btn-default"  onclick="location='${pageContext.request.contextPath}/cus/reminderTemp_toAdd.do'" value="新增">
...
<script>
    ...
    $(document).ready(function () {
	    ...
	    jQuery("#datagrid").jqGrid({
	       	url: '${pageContext.request.contextPath}/cus/reminderTemp_list_json.do',
	       	mtype: "GET",
...
//XXX tmh       	     		{ label: '序号', name: 'xxx', width: 150},
					...
					{ label: '抬头', name: 'first', width: 200},
      	     		{ label: '服务单位', name: 'serviceUnits', width: 150},
    ...
    function updateRecord(){
    	...
    	if(id==null || id==""){
    		...
    		$.get(
           			"${pageContext.request.contextPath}/cus/reminderTemp_checkReminderTempId.do?id="+id,
           			function(msg){
           				if(msg.result==true){
           						location.href='${pageContext.request.contextPath}/cus/reminderTemp_toUpdate.do?id='+id+'&pageNo='+pageNo;
       		            }else{
    		...
    		bootbox.confirm('确定需要删除吗？',
    		    function(result){
	    			if(result==true){  //执行删除操作
	    		    	$.post(
	    	           			"${pageContext.request.contextPath}/cus/reminderTemp_delete.do",
	    	           			{"ids":ids.join(","),"${_csrf.parameterName}":"${_csrf.token}"},
	    	           			function(msg){
	    	           				if(msg.result==true){
	    	           					bootbox.alert(msg.warnMsg, function(){
	    	    						    	window.location.href='${pageContext.request.contextPath}/cus/reminderTemp_list.do?pageNo='+pageNo;
	    	    						    }); 
	    	       		            }else{
	    	       		            	bootbox.alert(msg.warnMsg); 
	    	       		            	return false;
    		...
    		bootbox.confirm('确定需要发送吗？',
    		    function(result){
	    			if(result==true){  //执行发送操作
	    		    	$.get(
	    	           			"${pageContext.request.contextPath}/cus/reminderTemp_send.do?id="+id,
	    	           			function(msg){
	    	           				if(msg.result==true){
	    	           					bootbox.alert(msg.warnMsg, function(){
	    	    						    	window.location.href='${pageContext.request.contextPath}/cus/reminderTemp_list.do?pageNo='+pageNo;
    	...
    	if(id==null || id==""){
    		...
    		bootbox.confirm('确定需要发送吗？',
    		    function(result){
	    			if(result==true){  //执行发送操作
	    		    	$.get(
	    	           			"${pageContext.request.contextPath}/cus/reminderTemp_send_bind.do?id="+id,
	    	           			function(msg){
	    	           				/*if(msg.result==true){
	    	           					bootbox.alert(msg.warnMsg, function(){
	    	    						    	window.location.href='${pageContext.request.contextPath}/cus/reminder_list.do?pageNo='+pageNo;
	    	    						    }); 
	    	       		            }else{
	    	       		            	bootbox.alert(msg.warnMsg); 
	    	       		            	return false;
    ...
    function sentPreViewMethod(id,pageNo){
		...
		$.get(
       			"${pageContext.request.contextPath}/cus/reminderTemp_preview.do?id="+id,
       			function(msg){
       				if(msg.result==true){
       					alert(msg.warnMsg); 
       					window.location.href='${pageContext.request.contextPath}/cus/reminderTemp_list.do?pageNo='+pageNo;
