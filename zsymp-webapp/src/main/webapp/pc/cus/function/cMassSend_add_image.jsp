	  ...
	  <div class="MassSend_form">
	  	<form:form action="${pageContext.request.contextPath}/cus/cMassSend_add.do"  modelAttribute="CMassSendForm"  method="POST" >
		      ...
		      <input type="text" name="title" id="title"  class="general_input_class" size="64">
	      	  ...
	      	  <input type="button" id="previewRecord" class="btn btn-default" value="素材库">  <input type="button" id="deselectAllMatrial" value="清空重选">
...
<script>
    ...
    function setValuePicMtrl(obj){
    	if(obj.token=="system"){
    		...
    		$("#preview_box").append( '<div class="first_item"> '+'	<img src="'+"${pageContext.request.contextPath}"+finalPicPath+obj.picName+'" /> </div>');
    	    ++martrialNum;
    	}
    ...
    var martrialNum = 0;
     ...
     $(document).ready(function(){
	   ...
	   $("#deselectAllMatrial").click(function(){
		...
		if(martrialNum==1){
    		bootbox.alert("只能选择一条图片素材！");
			return false;
    	}else{
    		$('#picMtrlModal').modal('show');
    	} 
	   });
