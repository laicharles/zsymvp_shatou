...
<div class="wraper sidebar-full">
   <!-- 侧栏 -->
    ...
    <div class="container-fluid">
	  ...
	  <h2 class="title_class">编辑恢复供水通知</h2>
	  <form:form action="${pageContext.request.contextPath}/cus/noticeTemp_update.do"  modelAttribute="CNoticeTempForm"  method="POST" >
	      
	      	<input type="hidden" name="id" id="id" value="${noticeTempForm.id }" >
      	      	  
	      	  <div>
		      <label for="first" class="col-sm-2 control-label"><font color="red">*</font>抬头：</label>
		      <input type="text" name="first" id="first" value="${noticeTempForm.first }" class="general_input_class" size="128">
	      	  </div>
	      	  
	      	  <div>
		      <label for="noticeType" class="col-sm-2 control-label"><font color="red">*</font>恢复供水时间：</label>
		      <input type="text" name="noticeType" id="noticeType" value="${noticeTempForm.noticeType }" class="general_input_class" size="256">
	      	  </div>
	      	  
	      	  <div>
		      <label for="noticeContent" class="col-sm-2 control-label"><font color="red">*</font>恢复供水区域：</label>
	      	  <textarea rows="5" cols="20" name="noticeContent" id="noticeContent" size="512">${noticeTempForm.noticeContent }</textarea>
	      	  </div>
	      	  
	      	  <div>
		      <label for="remark" class="col-sm-2 control-label"><font color="red">*</font>其他说明：</label>
		      <textarea rows="5" cols="20" name="remark" id="remark" size="512">${noticeTempForm.remark }</textarea>
	      	  </div>
...
<script>
    ...
    $(document).ready(function(){
		$("#subBtn").click(function(){
		 ...
		 $("#returnBtn").click(function(){
			 window.location.href='${pageContext.request.contextPath}/cus/noticeTemp_list.do?pageNo=${page }';
			});
	  }); 
    ...
    function checkForm() { 
    	...
    	$("#CNoticeTempForm").children().filter("div:contains('*')").children().filter("input:text,textarea").each(function(){
		...
		$("#CNoticeTempForm").children().filter("div").children().filter("input:text,textarea").each(function(){
    ...
    function updateRecord(){
		if(checkForm()==true){
	    	bootbox.confirm("确认保存？",function(result){
	    		if(result==true){
				  $("#CNoticeTempForm").ajaxSubmit(function(msg){
							if(msg.result==false){
