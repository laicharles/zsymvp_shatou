...
<div class="wraper sidebar-full">
   <!-- 侧栏 -->
    ...
    <div class="container-fluid">
	  ...
	  <h2 class="title_class">编辑停水通知</h2>
	  <form:form action="${pageContext.request.contextPath}/cus/cStopWtrTemp_update.do"  modelAttribute="CStopWtrTempForm"  method="POST" >
	      
	      	<input type="hidden" name="id" id="id" value="${stopWtrTempForm.id }" >
      	
	          <div style="display:none;">
		      <label for="templateCode" class="col-sm-2 control-label"><font color="red">*</font>模板编号：</label>
		      <input type="text" name="templateCode" id="templateCode" readonly="readonly" value="${stopWtrTempForm.templateCode }">
	      	  </div>
	      	  
	          <div style="display:none;">
		      <label for="templateName" class="col-sm-2 control-label"><font color="red">*</font>模板名称：</label>
		      <input type="text" name="templateName" id="templateName" readonly="readonly" value="${stopWtrTempForm.templateName }">
	      	  </div>
	      	  
	          <div style="display:none;">
		      <label for="templateID" class="col-sm-2 control-label">模板ID：</label>
		      <input type="text" name="templateID" id="templateID" readonly="readonly" value="${stopWtrTempForm.templateID }">
	      	  </div>
	      	  
	      	  <div style="display:none;">
		      <label for=headColor class="col-sm-2 control-label">头部颜色：</label>
		      <input type="text" name="headColor" id="headColor" readonly="readonly" value="${stopWtrTempForm.headColor }">
	      	  </div>
	      	  
	      	  <div style="display:none;">
		      <label for="textColor" class="col-sm-2 control-label">文字颜色：</label>
		      <input type="text" name="textColor" id="textColor" readonly="readonly" value="${stopWtrTempForm.textColor }">
	      	  </div>
	      	  	      	  
	      	  <div>
		      <label for="first" class="col-sm-2 control-label"><font color="red">*</font>抬头：</label>
		      <input type="text" name="first" id="first" value="${stopWtrTempForm.first }" class="general_input_class" size="128">
	      	  </div>
	      	  
	      	  <div>
		      <label for="why" class="col-sm-2 control-label"><font color="red">*</font>停水类型：</label>
		      <input type="text" name="why" id="why" value="${stopWtrTempForm.why }" class="general_input_class" size="256">
	      	  </div>
	      	  
	      	  <div>
		      <label for="stopDuration" class="col-sm-2 control-label"><font color="red">*</font>停水时间：</label>
		      <input type="text" name="stopDuration" id="stopDuration" value="${stopWtrTempForm.stopDuration }" class="general_input_class" size="256">
	      	  </div>
	      	  
	      	  <div>
		      <label for="area" class="col-sm-2 control-label"><font color="red">*</font>停水区域：</label>
		      <textarea rows="5" cols="20" name="area" id="area" size="1024">${stopWtrTempForm.area }</textarea>
	      	  </div>
	      	  
	      	  <div>
		      <label for="tempRemark" class="col-sm-2 control-label"><font color="red">*</font>其他说明：</label>
		      <textarea rows="5" cols="20" name="tempRemark" id="tempRemark" size="512">${stopWtrTempForm.tempRemark }</textarea>
	      	  </div>
	      	  	      	  
	      	  <div>
		      <label for="url" class="col-sm-2 control-label">url：</label>
		      <input type="text" name="url" id="url" placeholder="http://www.wisewater.cn" value="${stopWtrTempForm.url }" class="pageUrl_class" size="256">
	      	  </div>
    ...
    $(document).ready(function(){
		$("#subBtn").click(function(){
		 ...
		 $("#returnBtn").click(function(){
			 window.location.href='${pageContext.request.contextPath}/cus/cStopWtrTemp_list.do?pageNo=${page }';
			});
	...
	if(checkForm()==true){
    	bootbox.confirm("确认保存？",function(result){
    		if(result==true){
					  ...
					  $("#CStopWtrTempForm").ajaxSubmit(function(msg){
								if(msg.result==false){
