...
<div class="wraper sidebar-full">
   <!-- 侧栏 -->
    ...
    <div class="container-fluid">
	  ...
	  <h2 class="title_class">编辑用户</h2>
	  
	  <form:form class="editForm" action="${pageContext.request.contextPath}/cus/bcustomeruser_update.do"  modelAttribute="BCustomerUserForm"  method="POST" >
	      
	      	  <input type="hidden" name="id" id="id" value="${userForm.id }">
	      	  <input type="hidden" name="role.roleCode1" id="role.roleCode" value="${userForm.role.roleCode }">
	        ...
	        <div>
		      ...
		      <select name="role.roleCode" class="general_input_class general_select_class">
		        <c:forEach items="${listForm }" var="from">
		        	<c:if test="${from.roleCode == userForm.role.roleCode}">
		        		<option value="${from.roleCode }" selected="selected">${from.roleName }</option>
		        	</c:if>
		        	<c:if test="${from.roleCode != userForm.role.roleCode}">
	        ...
	        <div>
		      ...
		      <input type="text" name="loginName" id="loginName" class="general_input_class"  readonly="readonly"  value="${userForm.loginName }" size="64">
	      	</div>
	      		
	      	<div>
		      <label for="mobile"  class="col-sm-2 control-label"><font color="red">*</font>手机号码：</label>
		      <input type="text"  name="mobile" id="mobile" class="general_input_class" value="${userForm.mobile }"  size="11">
	      	</div>
	      	
	      	<div>
		      <label for="userName"  class="col-sm-2 control-label"><font color="red">*</font>真实姓名：</label>
		      <input type="text"  name="userName" id="userName" class="general_input_class" value="${userForm.userName }"  size="64">
	      	</div>
	      	
	      	<div>
		      <label for="email"  class="col-sm-2 control-label"><font color="red">*</font>email：</label>
		      <input type="text"  name="email" id="email" class="general_input_class"  value="${userForm.email }"  size="64">
	      	</div>
	      	
	        <div>
	      	    <label for="nickName"><font color="red">*</font>自己微信昵称：</label>
                <input type="text" class="general_input_class" id="nickName" value="${cfanform.nickName}" size="64">
	      	</div>
	      	<div>
	      	   <input type="hidden" name="binOpendId" class="general_input_class" id="binOpendId" value="${userForm.binOpendId }"/>
	      	</div>
	      	
	      	<div>
	      	    <label for="bossLoginName"><font color="red">*</font>上级登录名：</label>
                <input type="text" class="general_input_class" id="bossLoginName" value="${bCustomerUser.loginName }" size="64"  placeholder="权限为最高级时可以空">
	      	</div>
	      	<div>
	      	    <input type="hidden" name="higherCustomerUserId" id="higherCustomerUserId" value="${userForm.higherCustomerUserId }"/>
	      	</div>
	      	...
	      	<select name="currentPermissions" class="general_input_class general_select_class">
		        <c:forEach items="${permissionForm }" var="from">
		        	<c:if test="${from.id == userForm.currentPermissions}">
		        		<option value="${from.id }" selected="selected">${from.reviewName }</option>
		        	</c:if>
		        	<c:if test="${from.id != userForm.currentPermissions}">
	      ...
	      <div>
		 	  ...
		 	  <textarea rows="5" cols="20" name="remarks" id="remarks" class="textArea_class" size="256">${userForm.remarks }</textarea>
	 	  </div>
     ...
     $(document).ready(function(){
	    ...
	    //失去焦点检查唯一性，上级登录名
	     $('#bossLoginName').blur(function(){
	    	 ...
	    	 if(this.value != ""){
 	    	   $("#bossLoginName").nextAll().remove(); 
	    	   $.get(
	          			"${pageContext.request.contextPath}/cus/findBossLoginName.do",
	          			{"bossName":this.value},
	          			function(msg){
	          				if(msg.loginName == bossLoginName){
	   	            	        $('#bossLoginName').after("<font id="+bossLoginName+"color color='green'>上级登录名可用！</font>");
	   	            	        console.log(msg.id);
	   	            	        $('#higherCustomerUserId').val(msg.id);
	   	            	    }else{
	   	            	    	$('#bossLoginName').after("<font id="+bossLoginName+"color color='red'>上级登录名不可用！</font>");
