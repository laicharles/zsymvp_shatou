...
<body>
	...
	<div class="wraper sidebar-full">
		...
		<div id="main">
			<div class="container-fluid auto-fluid">
					...
					<h2 class="title_class">编辑自动回复</h2>
					<form:form
						action="${pageContext.request.contextPath}/cus/cauto_update.do?_csrf=${_csrf.token}"
						modelAttribute="CAutoForm" method="POST"
						enctype="multipart/form-data">

						<input type="hidden" name="id" id="id" value="${cAutoForm.id}"> 
						
						<div id="tag_Div">
							<label for="tag"><font class="need_font">*</font>标签：</label>
							<div id="tagContent">
							<c:forEach items="${cAutoForm.CSelfTags}" var="tag" varStatus="tagStatus">
							<span id="tagSpan_${tagStatus.index}"><span  class="label label-success">${tag.name}</span>
							<span id="${tagStatus.index}" class="tagDelete" style="cursor:pointer">&times;</span>&nbsp;&nbsp;
							<input type="hidden" name="CSelfTags[${tagStatus.index}].id" value="${tag.id}">
							<input type="hidden" name="CSelfTags[${tagStatus.index}].name" value="${tag.name}">
							</span>
							</c:forEach>
						...
						<div id="keyWord_Div">
							...
							<div id="kwContent">
							<c:forEach items="${cAutoForm.CAutoKeywords}" var="kw" varStatus="kwStatus">
							
							<span id="kwSpan_${kwStatus.index}"><span  class="label label-info">${kw.keyName}</span>
							<span id="${kwStatus.index}" class="kwDelete" style="cursor:pointer">&times;</span>&nbsp;&nbsp;
							<input type="hidden" name="CAutoKeywords[${kwStatus.index}].id" value="${kw.id}">
							<input type="hidden" name="CAutoKeywords[${kwStatus.index}].keyName" value="${kw.keyName}">
							</span>
						...
						<div id="question_Div">
							<label for="question"><font class="need_font">*</font>问题名：</label> <input
								type="text" name="question" id="question" value="${cAutoForm.question}" class="general_input_class" size="100">
						</div>

						<div id="answerType_Div">
							<label for="answerType"><font class="need_font">*</font>回复类型：</label>
							<input type="hidden" name="answerTypeHd" id="answerTypeHd" value="${cAutoForm.answerType}">
						...
						<div class="form-group" id="textContent_Div">
							...
							<textarea class="textArea_class" name="textContent" size="300"
								id="textContent" >${cAutoForm.textContent}</textarea>
						</div>

						<div id="title_Div">
							<label for="title"><font class="need_font">*</font>图文标题：</label> <input
								type="text" name="title" id="title" value="${cAutoForm.title}" class="general_input_class" size="64">
						</div>

						<div id="picName_Div" class="form-group">
							<label for="picName">图文图片：</label>
							<div id="imgdiv">
								<img id="imgShow" class="imgShow_class" src="${pageContext.request.contextPath}/resources/attached/${token}/${cAutoForm.picUrl}"/>
							</div>
						...
						<div class="form-group" id="description_Div">
							...
							<textarea class="textArea_class"  name="description" size="300"
								id="description">${cAutoForm.description}</textarea>
						</div>

						<div id="pageUrl_Div">
							<label for="pageUrl">图文外链地址：</label>
							<input type="text" name="pageUrl" id="pageUrl" value="${cAutoForm.pageUrl}" class="pageUrl_class" size="256">
						</div>
						
						<div id="pageContent_Div">
							<label for="KindEditorId">正文：</label>
							<textarea name="KindEditor" id="KindEditorId" class="kindeditor_class">${cAutoForm.pageContent}</textarea>
							<input type="hidden" name="pageContent" id="pageContent">
						</div>
						
						<div id="author_Div">
							<label for="author">作者：</label>
							<input type="text" name="author" id="author" value="${cAutoForm.author}" class="general_input_class" size="32">
						</div>
						
						<div id="origUrl_Div">
							<label for="origUrl">原文链接：</label>
							<input type="text" name="origUrl" id="origUrl" value="${cAutoForm.origUrl}" class="pageUrl_class" size="256">
						</div>
	...
	<!-- 本页调用JS 开始 -->
	<script>
		var kwNum = ${cAutoForm.CAutoKeywords.size()};
		var tagNum = ${cAutoForm.CSelfTags.size()};
		$(function() {
			SideBar.init({
		...
		$(document).ready(function() {
	    	...
	    	$("#returnBtn").click(function(){
	    		window.location.href="${pageContext.request.contextPath}/cus/cauto_list.do?pageNo=${page }";
	    	});
			...
			$("#btn-tagAdd").click(function() {
				...
				$('#tagContent').prepend('<span id="tagSpan_'+tagNum+'"><span  class="label label-warning">'
										+ $("#tagAdd").val()
			...
			$("#btn-kwAdd").click(function() {
				...
				$('#kwContent').prepend('<span id="kwSpan_'+kwNum+'"><span  class="label label-warning">'
										+ $("#kwAdd").val()
						...
						success : function(result) {
							...
							$.each(
									...
									function(index, callback) {
										if ($("#answerTypeHd").val() == result[index].logicID) {
											...
											radioHtml += "<input type='radio' name='answerType' id='"+result[index].logicID+"' value='"+result[index].logicID+"' disabled>"
													+ result[index].dicName
													+ "&nbsp;&nbsp;";
										}
