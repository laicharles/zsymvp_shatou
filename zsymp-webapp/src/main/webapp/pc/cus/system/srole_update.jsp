...
<div class="wraper sidebar-full">
   <!-- 侧栏 -->
    ...
    <div class="container-fluid">
	  ...
	  <h2 class="title_class">修改角色</h2>
	  <form:form action="${pageContext.request.contextPath}/cus/srole_update.do"  modelAttribute="SRoleForm"  method="POST" >
	      
	      	  <input type="hidden" name="id" id="id" value="${roleForm.id }">
	          
	          <div>
		      <label for="roleCode"  ><font color="red">*</font>角色代码：</label>
		      <input type="text" name="roleCode" id="roleCode"  value="${roleForm.roleCode }" class="general_input_class" size="32">
	      	  </div>
	      		
	      	<div>
		      <label for="roleName"  class="col-sm-2 control-label"><font color="red">*</font>角色名：</label>
		      <input type="text"  name="roleName" id="roleName" value="${roleForm.roleName }" class="general_input_class" size="64">
	      	</div>
	      	
	      <div>
		 	  <label for="remarks" class="col-sm-2 control-label">备注：</label>
		 	  <textarea rows="5" cols="20" name="remarks" id="remarks" class="textArea_class" size="256">${roleForm.remarks }</textarea>
	 	  </div>
    ...
    function updateRecord(){
    	if(checkForm()==true){
	    	bootbox.confirm("确认保存？",function(result){
	    		if(result==true){
						  $("#SRoleForm").ajaxSubmit(function(msg){
									if(msg.result==false){
