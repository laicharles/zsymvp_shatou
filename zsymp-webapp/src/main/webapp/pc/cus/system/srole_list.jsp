...
<div class="wraper sidebar-full">
    ...
    <div class="container-fluid">
        <!--主体内容-->
        <h2 class="title_class">角色管理</h2>
        <div class="list_btn_div_class">
        <input type="button" id="addRecord"  class="btn btn-default"   onclick="location='${pageContext.request.contextPath}/cus/srole_toAdd.do'" value="新增">
        <input type="button" id="updateRecord"  class="btn btn-default" onclick="updateRecord()" value="编辑"> <input type="button" id="delRecord" class="btn btn-default" onclick="delRecord()" value="删除">
         <input type="button" id="delRecord"  class="btn btn-default" onclick="editAuthority()" value="权限配置">
         </div>
...
<script>
    ...
    $(document).ready(function () {
	    ...
	    jQuery("#datagrid").jqGrid({
	       	url: '${pageContext.request.contextPath}/cus/srole_list_json.do',
	       	mtype: "GET",
            contentType: "application/json",
            datatype: "json",
            colModel:[
      		       	{ label: '选择', name: 'id', key: true,hidden:true},
      	     		{ label: '角色代码', name: 'roleCode', width: 150},
      	     		{ label: '角色名', name: 'roleName', width: 250},
      	     		{ label: '备注', name: 'remarks', width: 400}
      	    ],
      	    viewrecords : true,
      	  	sortname : 'roleCode',
      	    sortable:true,
    ...
    function updateRecord(){
    	...
    	if(id==null || id==""){
    		...
    		$.get(
           			"${pageContext.request.contextPath}/cus/srole_checkRoleId.do?id="+id,
           			function(msg){
           				if(msg.result==true){
           						location.href='${pageContext.request.contextPath}/cus/srole_toUpdate.do?id='+id+'&pageNo='+pageNo;
       		            }else{
    		...
    		bootbox.confirm('确定需要删除吗？',
    		    function(result){
	    			if(result==true){  //执行删除操作
	    		    	$.post(
	    	           			"${pageContext.request.contextPath}/cus/srole_delete.do",
	    	           			{"ids":ids.join(","),"${_csrf.parameterName}":"${_csrf.token}"},
	    	           			function(msg){
	    	           				if(msg.result==true){
	    	           					bootbox.alert(msg.warnMsg, function(){
	    	    						    	window.location.href='${pageContext.request.contextPath}/cus/srole_list.do?pageNo='+pageNo;
	    	    						    }); 
	    	       		            }else{
	    	       		            	bootbox.alert(msg.warnMsg); 
	    	       		            	return false;
    ...
    function editAuthority(){
    	...
    	if(id==null || id==""){
    		...
    		bootbox.alert("选择了多行，请选择需要权限配置一条记录！")
    		return false;
    	}else{
    		...
    		$.get(
           			...
           			function(msg){
           				if(msg.result==true){
           					location.href='${pageContext.request.contextPath}/cus/srole_toAuthority.do?id='+id+'&pageNo='+pageNo;
       		            }else{
       		            	bootbox.alert(msg.warnMsg); 
       		            	return false;
       		            }
