...
<script>
    ...
    $(document).ready(function () {
	    jQuery("#datagrid").jqGrid({
	       	url: '${pageContext.request.contextPath}/cus/feedbackGrid.do?feedBackType=${feedBackType}',
	       	mtype: "GET",
            contentType: "application/json",
            datatype: "json",
            colModel:[
      		       	{ label: '选择', name: 'id', key: true,hidden:true},
      		      	{ label: '地址', name: 'addr', width: 100},
      	     		{ label: '反馈人', name: 'fanUserName', width: 100},
      	     		{ label: '联系电话', name: 'fanUserTel', width: 200},
      	     		{ label: '创建日期', name: 'createTime', width: 200},
      	     		{ label: '是否回复', name: 'isReply', width: 100},
      	     		{ label: '回复人', name: 'replyBy', width: 100},
      	     		{ label: '回复时间', name: 'replyTime', width: 200}
      	    ],
      	  	rownumbers:true,
      	    viewrecords : true,
	    	height:'100%',
    ...
    function editRecord(){
    	...
    	if(id==""||id==null){
    		...
    		location.href='${pageContext.request.contextPath}/cus/toUpdateFeedback.do?id='+id+'&pageNo='+pageNo+'&feedBackType=${feedBackType}';
    	}
    }
    //删除记录
    function delRecord(){
    	...
    	bootbox.confirm("确定删除吗？",function(result){
			if(result==true){
				$.ajax({
     	           type:"post",
     	           url:"${pageContext.request.contextPath}/cus/deleteFeedback.do",
     	           dataType:"json",
     	           async:false,
     	           ...
     	           success:function(resultList){
     	        	   if(resultList[0]=="success"){
     	        		  bootbox.alert("删除成功！",function(){
     	        			 ...
     	        			 window.location.href="${pageContext.request.contextPath}/cus/${feedBackType}/feedbackList.do?pageNo="+resultList[1];
     	        		  });
 			           }else{
