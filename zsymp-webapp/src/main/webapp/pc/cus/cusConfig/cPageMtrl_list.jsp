...
<div class="wraper sidebar-full">
    ...
    <div class="container-fluid">
        <!--主体内容-->
        <h2 class="title_class">文章素材列表</h2>
		<div class="list_btn_div_class">
        <input type="button" id="addRecord" class="btn btn-default"  onclick="location='${pageContext.request.contextPath}/cus/cPageMtrl_toAdd.do'" value="新增">
...
<script>
    ...
    $(document).ready(function () {
	    ...
	    jQuery("#datagrid").jqGrid({
	       	url: '${pageContext.request.contextPath}/cus/cPageMtrl_list_json.do',
	       	mtype: "GET",
...
//XXX tmh       	     		{ label: '序号', name: 'xxx', width: 150},
      	     		...
      	     		{ label: '创建人', name: 'createdBy', width: 400},
  ...
  //查询方法
    ...
    function updateRecord(){
    	...
    	if(id==null || id==""){
    		...
    		$.get(
           			"${pageContext.request.contextPath}/cus/cPageMtrl_checkMtrlId.do?id="+id,
           			function(msg){
           				if(msg.result==true){
           						location.href='${pageContext.request.contextPath}/cus/cPageMtrl_toUpdate.do?id='+id+'&pageNo='+pageNo;
       		            }else{
    		...
    		bootbox.confirm('确定需要删除吗？',
    		    function(result){
	    			if(result==true){  //执行删除操作
	    		    	$.post(
	    	           			"${pageContext.request.contextPath}/cus/cPageMtrl_delete.do",
	    	           			{"ids":ids.join(","),"${_csrf.parameterName}":"${_csrf.token}"},
	    	           			function(msg){
	    	           				if(msg.result==true){
	    	           					bootbox.alert(msg.warnMsg, function(){
	    	    						    	window.location.href='${pageContext.request.contextPath}/cus/cPageMtrl_list.do?pageNo='+pageNo;
	    	    						    }); 
	    	       		            }else{
	    	       		            	bootbox.alert(msg.warnMsg); 
	    	       		            	return false;
