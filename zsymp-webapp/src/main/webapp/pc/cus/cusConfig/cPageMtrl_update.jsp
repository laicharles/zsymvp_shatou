...
<div class="wraper sidebar-full">
   <!-- 侧栏 -->
    ...
    <div class="container-fluid">
	  ...
	  <h2 class="title_class">编辑文章素材</h2>
	  <form:form action="${pageContext.request.contextPath}/cus/cPageMtrl_update.do"  modelAttribute="CPageMaterialForm"  method="POST" >
	        ...
	        <div>
	      	...
	      	<div>
				...
				<textarea name="KindEditor" id="KindEditor" class="kindeditor_class">${mtrlForm.pageContent }</textarea>
				</div>
			...
			<button class="btn btn-default" data-toggle="modal"
	      	...
	      	<input type="button" class="btn btn-default"  id="returnBtn" value="返回"> <input type="button" id="subBtn"  class="btn btn-default" value="保存" onmouseover="getContent()"> 
...
<script>
    ...
    function setValuePageMtrl(obj){
    	$("#title").val(obj.title);
    	$("#pageContent").val(obj.pageContent);
    ...
    $(document).ready(function(){
		$("#subBtn").click(function(){
		 ...
		 $("#returnBtn").click(function(){
			 window.location.href='${pageContext.request.contextPath}/cus/cPageMtrl_list.do?pageNo=${page }';
			});
    ...
    function checkForm() { 
    	...
    	$("#CPageMaterialForm").children().filter("div:contains('*')").children().filter("input:text,textarea").each(function(){
		...
		$("#CPageMaterialForm").children().filter("div").children().filter("input:text,textarea").each(function(){
	...
	if(checkForm()==true){
    	bootbox.confirm("确认保存？",function(result){
    		if(result==true){
					  ...
					  $("#CPageMaterialForm").ajaxSubmit(function(msg){
								if(msg.result==false){
