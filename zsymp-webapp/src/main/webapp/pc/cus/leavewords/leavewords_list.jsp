...
<script>
    ...
    $(document).ready(function () {
	    jQuery("#datagrid").jqGrid({
	       	url: '${pageContext.request.contextPath}/cus/leavewordsGrid.do?leavewords=${leavewords}',
	       	mtype: "GET",
            contentType: "application/json",
            datatype: "json",
            colModel:[
      		       	{ label: '选择', name: 'id', key: true,hidden:true},
      		      	{ label: 'openId', name: 'openId', width: 200},
      	     		{ label: '昵称', name: 'nickName', width: 200},
      	     		{ label: '创建日期', name: 'createTime', width: 200},
      	     		{ label: '是否回复', name: 'isReply', width: 200},
    ...
    function editRecord(){
    	...
    	if(id==""||id==null){
    		...
    		location.href='${pageContext.request.contextPath}/cus/toUpdateLeavewords.do?id='+id+'&pageNo='+pageNo+'&leavewords=${leavewords}';
    	}
    }
    //删除记录
    function delRecord(){
    	...
    	bootbox.confirm("确定删除吗？",function(result){
			if(result==true){
				$.ajax({
     	           type:"post",
     	           url:"${pageContext.request.contextPath}/cus/deleteLeavewords.do",
     	           dataType:"json",
     	           async:false,
     	           ...
     	           success:function(resultList){
     	        	   if(resultList[0]=="success"){
     	        		  bootbox.alert("删除成功！",function(){
     	        			 ...
     	        			 window.location.href="${pageContext.request.contextPath}/cus/${leavewords}/leavewordsList.do?pageNo="+resultList[1];
     	        		  });
 			           }else{
 			        	  bootbox.alert("删除失败！"+resultList[0],function(){
 			        		 window.location.href="${pageContext.request.contextPath}/cus/${leavewords}/leavewordsList.do?pageNo="+resultList[1];
      	        		  });
 			           }
     	           }
