...
<div class="wraper sidebar-full">
    ...
    <div class="container-fluid">
        <h2 class="title_class">编辑我要留言</h2>
		<form:form action="${pageContext.request.contextPath}/cus/updateLeavewords.do?_csrf=${_csrf.token}" modelAttribute="CLeaveWordsManageForm" method="POST" enctype="multipart/form-data">
			<input type="hidden" name="id" id="id" value="${leavewordsForm.id }">
			<input type="hidden" name="token" value="${leavewordsForm.token }">
			<input type="hidden" name="token" value="${leavewordsForm.openId }">
			<!-- 
			<div>
				<label for="nameId">户名：</label>
				<input type="text" name="customerType" id="customerTypeId" value="${leavewordsForm.name }" class="general_input_class" readonly>
			</div>
			
			<div>
				<label for="telId">电话：</label>
				<input type="text" id="telId" value="${leavewordsForm.tel }" class="general_input_class" readonly>
			</div>
			<div>
				<label for="addrId">地址：</label>
				<input type="text" id="addressId" value="${leavewordsForm.address }" class="pageUrl_class" readonly>
			</div>
			 -->
			<div>
				<label for="descriptionId">用户留言：</label>
				<textarea id="contentId" class="textArea_class pageUrl_class" readonly> ${leavewordsForm.content }</textarea>
			</div>
			<!-- 
			<div>
				<label for="imgListId">附件：</label>
				<c:forEach items="${picNameList }" var="picName">
					<a class="example-image-link" href="${pageContext.request.contextPath}/resources/attached/${token}/leaveword/${picName }" data-lightbox="example-set" >
						<img class="example-image" src="${pageContext.request.contextPath}/resources/attached/${token}/leaveword/${picName }" alt="" style="width:100px;"/>
					</a>
				</c:forEach>
				<c:if test="${empty picNameList }">
					无照片！
				</c:if>
			</div>
			 -->
			...
			<div>
				...
				<input type="text" name="replyBy" id="replyById" value="${leavewordsForm.replyBy }" class="general_input_class">
			</div>
			 <div>
				<label for="KindEditorId" class="col-sm-2 control-label"><font color="red">*</font>回复正文：</label>
				<div class="pageContent_class">
				<textarea name="KindEditor" id="KindEditorId" class="kindeditor_class">${leavewordsForm.replyContent }</textarea>
				</div>
    ...
    $(function(){
    	...
    	$("#nameId").blur(function(){
    		...
    		if(isNotNull(name)==false){
    			name.after("<font id='nameWarn' color='red'>请填写名称！</font>");
    			name.focus();
    	...
    	*/
    	...
    	$("#descriptionId").blur(function(){
    		...
    		if(isNotNull(description)==false){
    			description.after("<font id='descriptionWarn' color='red'>请填写留言！</font>");
    			description.focus();
        	}else if(chklength(description,1024)==false){
        		description.after("<font id='descriptionWarn'  color='red'>请输入小于512个字的留言！</font>");
        		description.focus();
        	}
    	});
    	...
    	*/
    	...
    	$("#returnBtn").click(function(){
    		window.location.href="${pageContext.request.contextPath}/cus/${leavewords}/leavewordsList.do?pageNo=1";
    	});
    	//保存
    	$("#saveBtn").click(function(){
    		if(true){
    			bootbox.confirm("确定保存吗？",function(result){
    				if(result==true){
    					$("#CLeaveWordsManageForm").ajaxSubmit(function(msg){
               				if(msg.result==false){
