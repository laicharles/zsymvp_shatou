...
<%-- 		<p>${token}</p>
	...
	<script>
		...
		$loadMore.on('click', function () {
			...
			setBtnStatus(2); // 加载中...
			$.ajax({
				url: '${pageContext.request.getContextPath()}/m/feedbackListJson.do',
				dataType: 'json',
				async: true,
				data: {
					'token': '${token}',
			    ...
			    success: function (data) {
					if (data == null || data.length == 0 || data == undefined) {
						...
						for (var i = 0, len = data.length; i < len; i++) {
							if (data[i].isReply == 1) {
								html += '<li onclick="viewDetail(\'' + data[i].id + '\',\''+data[i].token+'\')" class="answered">'
									+		'<p>' + data[i].fanUserName + '<br>'
								...
								html += '<li onclick="viewDetail(\'' + data[i].id + '\',\''+data[i].token+'\')" id="viewBtn">'
									+		'<p>' + data[i].fanUserName + '<br>'
									+			'<span class="time">' + data[i].createTime + '</span>'
									+		'</p>'
									+		'<div class="arrow-wrapper">'
		...
		function viewDetail(id, token) {
			window.location.href = '${pageContext.request.contextPath}/m/feedbackView.do?id=' + id+'&token='+token;	
		}
